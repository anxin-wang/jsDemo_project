...
var right=$('.right'),
	fixed_nav=$('.fixed-nav');
function fixed_show(){
	var right_offset=right.offset().left;
	var ie6_right=right.position().left;
	if($(window).scrollTop()>150){
		if(!window.XMLHttpRequest){
			fixed_nav.css('left',ie6_right).fadeIn(600);
		}else{
			fixed_nav.css('left',right_offset).fadeIn(600);
		}
	}else{
...
var menu_outer=$('#down_menu'),
		d_menu=$('#d_menu');
menu_outer.hover(function(){
		d_menu.show();
	},function(){
		d_menu.hide();
});

$('#close_fixed_nav').click(function(){
	fixed_nav.hide();
	fixed_show=function(){}
});